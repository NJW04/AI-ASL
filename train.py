"""Train a configurable CNN on the ASL Alphabet dataset.

Features
--------
- From-scratch CNN (configurable blocks/activation).
- Early stopping on validation macro-F1.
- Timestamped run folders with saved artifacts and logs.
"""
...
try:
    ...
    tqdm = None  # Fallback handled in __main__
...
def _timestamp_slug(slug: str) -> str:
    """Prefix a slug with a timestamp like ``YYYY-MM-DD_HH-MM-SS``."""
...
def create_run_dir(artifacts_root: Path, slug: str) -> Path:
    ...
    Example
    -------
    ``artifacts/asl_runs/2025-10-27_14-05-33__train-cnn_small-lr0.001-b64-ep15``
    """
...
def copy_to(src: Path, dst: Path):
    """Copy a file from ``src`` to ``dst``, creating parent dirs if needed."""
    dst = Path(dst)
...
def log_banner(logger: logging.Logger, title: str):
    """Log a banner-styled section header."""
    line = "=" * (len(title) + 4)
    logger.info("\n%s\n| %s |\n%s", line, title, line)
...
def set_seed(seed: int = 42, deterministic: bool = True):
    ...
    If ``deterministic`` is True, configures cuDNN for determinism.
    """
    random.seed(seed)
    np.random.seed(seed)
...
def evaluate(model: nn.Module, loader: DataLoader, device: torch.device) -> Tuple[float, float, np.ndarray, np.ndarray, List[str], List[str]]:
    """Evaluate a model on a dataloader and return loss and predictions."""
    model.eval()
...
def run_training(args) -> Dict:
    """Run the full training pipeline and return summary metadata."""
    ...
    if args.aug:
        slug_parts.append("aug")
    if args.size != 96:
        slug_parts.append(f"sz{args.size}")
    if args.subset_per_class:
        slug_parts.append(f"sub{args.subset_per_class}")
    ...
    logger.info("Model: %s (blocks=%d, act=%s)",
                model.__class__.__name__, args.num_blocks, args.activation)
...
def parse_args(args_list=None):
    """Parse command-line arguments (or a provided list)."""
...
if __name__ == "__main__":
    if tqdm is None:
        ...
        class tqdm:
            """Minimal tqdm stub used when tqdm is not installed."""
