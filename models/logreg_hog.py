...
# --- HOG-specific import (assuming this path is correct for your project) ---
...
def _extract_feature_vec(img_path: str) -> np.ndarray:
    """
    Extracts a combined HOG (shape) and Color (RGB) histogram feature vector
    using the custom project transform.
    """
...
def main():
    ...
    run_dir = Path(args.artifacts_root) / f"{ts}-baseline-hog" 
    ensure_dir(run_dir)
    logger = get_logger(run_dir)
    log_banner(logger, "BASELINE: HOG (SGD) WITH PER-EPOCH METRICS") 
    ...
    # --- UPDATED CACHE SIGNATURE FOR HOG ---
    # If you change your extract_hog_color function, 
    # delete the cache or update this string
    feature_sig = "hog_color_default"
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if f1_va > best_val_f1:
            ...
            best_epoch = epoch
            epochs_no_improve = 0
            best_clf = copy.deepcopy(clf) # Save a deep copy of the best *fitted* model
    ...
    def _eval(X, y, split_name: str):
        ...
        plot_confusion(y, pred, labels=class_names, out_path=run_dir / f"confmat_{split_name}.png",
                       title=f"HOG(SGD) Confusion ({split_name})") # <-- Updated title
