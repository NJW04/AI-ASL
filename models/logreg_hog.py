"""HOG + RGB color histogram baseline with cached features and SGD training.
...
def _extract_feature_vec(img_path: str) -> np.ndarray:
    """Extract HOG (shape) + RGB color histogram features for one image."""
...
def main():
    """Train and evaluate the HOG+Color baseline with per-epoch logging and early stopping."""
    ap = argparse.ArgumentParser(
        description="Baseline: SGD(Logistic) on HOG + Color features with per-epoch metrics"
    )
    ...
    def _eval(X, y, split_name: str):
        ...
        plot_confusion(
            ...
            title=f"HOG(SGD) Confusion ({split_name})",
        )
