...
- predictions_val.csv / predictions_test.csv (columns: path,true,pred,top3)
...
def _timestamp_slug(slug: str) -> str:
    """Return timestamp-prefixed slug (``YYYY-MM-DD_HH-MM-SS__slug``)."""
    ts = _dt.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    return f"{ts}__{_slugify(slug)}" if slug else ts


def create_run_dir(artifacts_root: Path, slug: str) -> Path:
    """Create and return a run directory under ``artifacts_root``."""
...
def _infer_model_type(checkpoint_path: Path) -> str:
    """Infer model type from sibling ``best.json``/``config.json`` if present."""
...
def _build_model(model_name: str, num_classes: int,  # noqa: D401
                 num_blocks: int, activation: str, dropout: float):
    """Build the CNNSmall model with the trained architecture parameters."""
...
def _softmax_topk(logits: torch.Tensor, k=3):
    """Return top-k softmax probabilities and indices."""
...
def evaluate_split(model: nn.Module, loader, class_names: List[str], device: torch.device, out_csv: Path):
    """Evaluate a dataloader, save per-sample predictions, and return arrays of truths/preds."""
    model.eval()
...
def main():
    """Evaluate a checkpoint on validation and (optionally) test splits."""
    ...
    logger.info(
        "Using model params: num_blocks=%d, activation=%s, dropout=%.4f",
        model_num_blocks,
        model_activation,
        model_dropout,
    )
    ...
    y_true, y_pred = evaluate_split(
        model, val_loader, class_names, device, out_csv=run_dir / "predictions_val.csv"
    )
    metrics = compute_metrics(y_true, y_pred, class_names)
    write_json(metrics, run_dir / "metrics_val.json")
    plot_confusion(
        y_true, y_pred, class_names, out_path=run_dir / "confmat_val.png", title="Confusion (val)"
    )
    logger.info("VAL: acc=%.4f macroF1=%.4f", metrics["accuracy"], metrics["macro_f1"])

    if test_loader is not None:
        y_true_t, y_pred_t = evaluate_split(
            model, test_loader, class_names, device, out_csv=run_dir / "predictions_test.csv"
        )
        metrics_t = compute_metrics(y_true_t, y_pred_t, class_names)
        write_json(metrics_t, run_dir / "metrics_test.json")
        plot_confusion(
            y_true_t, y_pred_t, class_names, out_path=run_dir / "confmat_test.png", title="Confusion (test)"
        )
